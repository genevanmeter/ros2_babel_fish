...
BabelFishService::BabelFishService( std::shared_ptr<rcl_node_t> node_base,
  ...
  : ServiceBase( std::move( node_base )), type_support_( std::move( type_support )), callback_( callback )
{
...
{
  rcl_ret_t ret = rcl_send_response( get_service_handle().get(), &request_id, response.type_erased_message().get());
...
void BabelFishService::handle_request( std::shared_ptr<rmw_request_id_t> request_header, std::shared_ptr<void> request )
{
