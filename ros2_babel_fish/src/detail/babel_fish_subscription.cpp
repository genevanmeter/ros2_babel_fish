...
BabelFishSubscription::BabelFishSubscription( rclcpp::node_interfaces::NodeBaseInterface *node_base,
  ...
  : rclcpp::SubscriptionBase( node_base, type_support->type_support_handle, topic_name,
                              create_subscription_options( qos ),
                              false )
...
void BabelFishSubscription::handle_message( std::shared_ptr<void> &message, const rclcpp::MessageInfo &message_info )
{
  callback_.dispatch( CompoundMessage::make_shared( *type_support_, message ), message_info );
}

void BabelFishSubscription::handle_loaned_message( void *loaned_message, const rclcpp::MessageInfo &message_info )
{
  // Not handled
  (void) loaned_message;
  (void) message_info;
...
void BabelFishSubscription::handle_serialized_message(const std::shared_ptr<rclcpp::SerializedMessage> & message, const rclcpp::MessageInfo & message_info)
{
  // Not handled
  (void) message;
  (void) message_info;
...
bool BabelFishSubscription::take( CompoundMessage &message_out, rclcpp::MessageInfo &info_out )
{
  ...
  if ( type_erased == nullptr || !take_type_erased( type_erased.get(), info_out )) return false;
...
std::string BabelFishSubscription::get_message_type() const
{
  return type_support_->name;
}
}
